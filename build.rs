...
const LIBRARY_DLL: &str = "AnselSDK64.dll";
const RUNTIME_DLL: &str = "AnselSDK64.bak";
...
fn write_asm_x64(exports: &[&str]) {
    let mut out = File::create("src/ansel.x64.S").expect("Failed to create src/ansel.x64.S");
...
fn write_asm_x86(exports: &[&str]) {
    let mut out = File::create("src/ansel.x86.S").expect("Failed to create src/ansel.x86.S");
