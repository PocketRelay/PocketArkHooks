...
impl Pattern {
    ...
    /// * action - The action to take on the memory
    pub unsafe fn apply_with_transform<F, T>(&self, length: usize, transform: T, action: F)
    where
        T: FnOnce(*const u8) -> *const u8,
        F: FnOnce(*mut u8),
    {
        let Some(addr) = self.find() else {
            eprintln!("Failed to find {} hook position", self.name);
    ...
    unsafe fn use_memory<F>(addr: *const u8, length: usize, action: F)
    where
        F: FnOnce(*mut u8),
    {
